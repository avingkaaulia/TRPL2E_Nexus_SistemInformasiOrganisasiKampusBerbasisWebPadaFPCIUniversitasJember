...
@section('title', 'Edit Anggota - Admin FPCI UNEJ')
@section('page-title', 'Edit Anggota')

@section('content')
<div class="admin-card">
    <div class="admin-card-header">
        <h4><i class="bi bi-pencil-square me-2"></i> Form Edit Anggota</h4>
    ...
    <form action="{{ route('admin.anggota.update', $anggota->id_anggota) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror" value="{{ old('nama', $anggota->user->nama) }}" required>
                    @error('nama')
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="text" name="username" class="form-control @error('username') is-invalid @enderror" value="{{ old('username', $anggota->user->username) }}" required>
                    @error('username')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <select name="id_divisi" class="form-select @error('id_divisi') is-invalid @enderror" required>
                        ...
                        <option value="{{ $divisi->id_divisi }}" {{ old('id_divisi', $anggota->id_divisi) == $divisi->id_divisi ? 'selected' : '' }}>
                            {{ $divisi->nama_divisi }}
                        </option>
                        @endforeach
                    </select>
        ...
        <div class="row">
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <select name="periode" class="form-select @error('periode') is-invalid @enderror" required>
                        ...
                        <option value="{{ $periode }}" {{ old('periode', $anggota->periode) == $periode ? 'selected' : '' }}>
                            {{ $periode }}
                        </option>
                        @endforeach
                    </select>
            ...
            <div class="col-md-6">
                <div class="mb-3">
                    ...
                    <input type="number" name="no_urut" class="form-control @error('no_urut') is-invalid @enderror" value="{{ old('no_urut', $anggota->no_urut) }}" required>
                    @error('no_urut')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="mb-3">
            ...
            <input type="text" name="link" class="form-control" value="{{ old('link', $anggota->link) }}">
        </div>
        ...
        <div class="mb-3">
            ...
            <div>
                @if($anggota->foto)
                    <img src="{{ asset($anggota->foto) }}" class="img-thumbnail" width="100">
                @else
